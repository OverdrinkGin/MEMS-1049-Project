...
int Plug;//手柄数据相关标志
float Velocity_KP =1.4, Velocity_KI =0.06;//PI参数
int Flag_Way=1,Flag_Direction;             //遥控模式标志与蓝牙窜口通信相关标志
int PS2_Velocity,Battery_Voltage,Bluetooth_Velocity =450; //电池电压采样变量和上位机相关变量
...
{if(Flag_Direction == 1||Flag_Direction == 5)PWMA=-Gryo*Bluetooth_Velocity/3300,PWMB=-Gryo*Bluetooth_Velocity/3300,PWMC=+Gryo*Bluetooth_Velocity/3300,PWMD=+Gryo*Bluetooth_Velocity/3300;
 }
/**************************************************************************
函数功能：异常关闭电机
入口参数：电压
...
void control() {
  ...
  sei();//全局中断开启
  PS2_Velocity =3.5*(abs(LY)+abs(LX)/2+abs(RX));//取摇杆数据为目标速度与摇杆力度和角度相关
  Get_RC();//遥控函数
  if(Flag_Way == 1){//蓝牙遥控赋值
  Multiple=4*Bluetooth_Velocity/80;//目标速度大，加快启动加速过程
...
void Get_RC() {
  if (Transformation == 0) {//进入APP摇杆模式标志
     if (Flag_Direction == 0){VelocityA =0;VelocityB =0;VelocityC =0;VelocityD =0;}//停止，速度为0
        ...
        VelocityA+=15;if(abs(VelocityA)>Bluetooth_Velocity) VelocityA= Bluetooth_Velocity;//赋予设定的目标速度，轮子前转
        VelocityB+=15;if(abs(VelocityB)>Bluetooth_Velocity) VelocityB= Bluetooth_Velocity;//赋予设定的目标速度，轮子前转
        VelocityC+=15;if(abs(VelocityC)>Bluetooth_Velocity) VelocityC= Bluetooth_Velocity;//赋予设定的目标速度，轮子前转
        VelocityD+=15;if(abs(VelocityD)>Bluetooth_Velocity) VelocityD= Bluetooth_Velocity;//赋予设定的目标速度，轮子前转
      }
      ...
      else if (Flag_Direction == 7) {//左自转
        VelocityA-=15 ;if(abs(VelocityA)>Bluetooth_Velocity) VelocityA= -Bluetooth_Velocity;//赋予设定的目标速度，轮子前转
        VelocityB-=15 ;if(abs(VelocityB)>Bluetooth_Velocity) VelocityB= -Bluetooth_Velocity;//赋予设定的目标速度，轮子前转
        VelocityC+=15 ;if(abs(VelocityC)>Bluetooth_Velocity) VelocityC= Bluetooth_Velocity;//赋予设定的目标速度，轮子前转
        VelocityD+=15 ;if(abs(VelocityD)>Bluetooth_Velocity) VelocityD= Bluetooth_Velocity;//赋予设定的目标速度，轮子前转
      }
      else if (Flag_Direction == 3) {//右自转
        VelocityA+=15;if(abs(VelocityA)>Bluetooth_Velocity) VelocityA= Bluetooth_Velocity;//赋予设定的目标速度，轮子前转
        VelocityB+=15;if(abs(VelocityB)>Bluetooth_Velocity) VelocityB= Bluetooth_Velocity;//赋予设定的目标速度，轮子前转
        VelocityC-=15;if(abs(VelocityC)>Bluetooth_Velocity) VelocityC= -Bluetooth_Velocity;//赋予设定的目标速度，轮子后转
        VelocityD-=15;if(abs(VelocityD)>Bluetooth_Velocity) VelocityD= -Bluetooth_Velocity; //赋予设定的目标速度，轮子后转
      }
      else if (Flag_Direction == 8) {//左转弯
        VelocityA = Bluetooth_Velocity/2;//赋予设定的目标速度，轮子前转
        VelocityB = Bluetooth_Velocity/2;
        VelocityC = Bluetooth_Velocity;//赋予设定的目标速度，轮子前转
        VelocityD = Bluetooth_Velocity;
      }
      else if (Flag_Direction == 2) {//右转弯
        VelocityA = Bluetooth_Velocity;                 
        VelocityB = Bluetooth_Velocity;//赋予设定的目标速度，轮子前转
        VelocityC = Bluetooth_Velocity/2;
        VelocityD = Bluetooth_Velocity/2;//赋予设定的目标速度，轮子前转
      }
      else if (Flag_Direction == 6) {//左后转弯
        VelocityA = -Bluetooth_Velocity/2;                 
        VelocityB = -Bluetooth_Velocity/2;//赋予设定的目标速度，轮子后转
        VelocityC = -Bluetooth_Velocity;
        VelocityD = -Bluetooth_Velocity;//赋予设定的目标速度，轮子后转
      }
      else if (Flag_Direction == 4) {//右后转弯
        VelocityA = -Bluetooth_Velocity;//赋予设定的目标速度，轮子后转
        VelocityB = -Bluetooth_Velocity;
        VelocityC = -Bluetooth_Velocity/2;//赋予设定的目标速度，轮子后转
        VelocityD = -Bluetooth_Velocity/2;
      }
  ...
  if (Transformation == 1) {//进入APP按键模式标志
    ...
    if (Flag_Way == 0)   {//PS2控制
    if(LY<=0){
    Velocity__A = 3.5*(+LX/2-LY+RX/1.5);//手柄运动控制函数，3.5为设定摇杆速度倍率，可根据需要调整
    Velocity__B = 3.5*(+LX/2-LY+RX/1.5);//手柄运动控制函数，3.5为设定摇杆速度倍率，可根据需要调整
    Velocity__C = 3.5*(-LX/2-LY-RX/1.5);//手柄运动控制函数，3.5为设定摇杆速度倍率，可根据需要调整
    Velocity__D = 3.5*(-LX/2-LY-RX/1.5);//手柄运动控制函数，3.5为设定摇杆速度倍率，可根据需要调整
    }
    else if(LY>0){
    Velocity__D = 3.5*(+LX/2-LY+RX/1.5);//手柄运动控制函数，3.5为设定摇杆速度倍率，可根据需要调整
    Velocity__C = 3.5*(+LX/2-LY+RX/1.5);//手柄运动控制函数，3.5为设定摇杆速度倍率，可根据需要调整
...
void OLED()
{
  ...
  if(Flag_Way == 0){oled.drawstring(00, 3, "Target-Velocity:");//显示手柄设定的目标速度
   ...
   if(Flag_Way == 1){oled.drawstring(95, 04, "APP");}
...
void loop() {  
  ...
  if(PS2_RY<120){Flag_Way = 1;}//手柄右摇杆上拨，进入APP模式
  if(PS2_RY==255&&PS2_RX==255&&PS2_LY==255&&PS2_LX==255){Flag_Way = 1;Plug=1;}//拔掉手柄接收模块，进入APP模式
...
unsigned char  select(void){
  ...
  PS2_LX=ps2x.Analog(PSS_LX);//手柄左摇杆X轴数据读取
  PS2_RY=ps2x.Analog(PSS_RY);//手柄右摇杆Y轴数据读取
  if(PS2_RY==255&&PS2_RX==255&&PS2_LY==255&&PS2_LX==255){Flag_Way = 1;Plug=1;flag=0;}//不插手柄接收模块，禁止进入手柄模式，直接进入APP模式
